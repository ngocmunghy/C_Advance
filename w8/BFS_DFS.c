...
int getAdjacentVertices(Graph graph,int vertice,int *output);
...
void addEdge(Graph graph,int v1,int v2)
{
    ...
    if(!adjacent(graph,v1,v2))
    {
        ...
        jrb_insert_int(tree,v2,new_jval_v(1));  
    }
...
int getAdjacentVertices(Graph graph,int vertice,int output[])
{
    JRB node,tree;
    int index = 0;
    node = jrb_find_int(graph,vertice);
    if(node == NULL)
        return 0;
    tree = (JRB)jval_v(node->val);
    jrb_traverse(node,tree)
    {
        output[index++] = jval_i(node->key);
    }
